...
function initDashboard() {
  ...
  const haulingCount = 0; // pwede mong i-link sa hauling module
  const earnings = 0;     // pwede mong i-link sa earnings module
  ...
  if (ctx) {
    yardChart = new Chart(ctx, {
      ...
      options: {
        responsive: true,
        maintainAspectRatio: false, // ✅ Added here
        plugins: {
  ...
  // Clear tables (no data yet)
  document.getElementById("scheduleBody").innerHTML =
    "<tr><td colspan='3' style='text-align:center;'>No hauling schedule available</td></tr>";
  document.getElementById("activityBody").innerHTML =
    "<tr><td colspan='3' style='text-align:center;'>No recent activities yet</td></tr>";
}
